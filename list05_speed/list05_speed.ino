...
void loop(){
    ...
    int speed = 255;    /* サーボモータのスピード制御用変数 */
    /* 事前に測定したデータを代入する配列(テスト用にわかりやすい値を代入しています。) */
    int data_theta1[10] = {180,160,140,120,100,80,60,40,20,0};
    int data_theta2[10];
    int data_theta3[10];
    ...
    while(1){
        ...
        case 31:
            /* 配列からのデータの読み出し */
            for(i=0;i<10;i++){
                mServo1.write(data_theta1[i],speed,true);            
                /* 実験の都合上,コメントしている. */
                // mServo2.write(data_theta2[i],speed,true);
                // mServo3.write(data_theta3[i],speed,true);
            }
        ...
        case 42:
            Serial.println("Value & Switch check");
            mode = 43;
            break;
        case 43:
